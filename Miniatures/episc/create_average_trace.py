...
data = pd.read_excel("./output_for_paper/mepsc_all.xlsx")
...
def get_original_trace(data):
    sr = 50000.0
    rsr = 5000.0
    full_length = 0.06 * rsr
    padding = 0.02 * rsr
    out = []
    # data = data.loc[(data.amplitude < np.percentile(data.amplitude, 75)) &
    #                 (data.amplitude > np.percentile(data.amplitude, 25)) &
    #                 (data.tau < np.percentile(data.tau, 75)) &
    #                 (data.tau > np.percentile(data.tau, 40))]
    for d in data.id.unique():
        trace = get_abf(d).astype(float)
        trace = np.hstack(np.array(trace))
        trace, _ = resample_data(trace, 5000, 50000)
        trace = filtering(trace, 250, "lowpass", 5000)
        ...
        for i, r in cell_data.iterrows():
            ...
            t = np.array(trace[start:end])[0:400]
            out.append(t)
    out = np.array(out)
    return out
...
def make_fig(data, dataset, fig=None, ax=None):
    ...
    f_above = f + f_sem
    f_below = f - f_sem
    ax.plot(np.linspace(-20, (len(f)/5)-20, len(f)), f, color=c)
    # ax.fill_between(np.linspace(-20, 60, len(f)), f_above, f_below,
    #                 color=c, alpha=0.5)
    return fig, ax
