...
def generate_filenames(meta, data_loc):
    for index, row in meta.iterrows():
        ...
        epsc = "-".join([" ".join([row["name"], str(row["cell"])]),
                         str(row["epsc"])])
        epsc = os.path.join(data_loc, epsc) + ".ABF"
        meta.loc[index, "seal"] = seal
        meta.loc[index, "epsc"] = epsc
    return meta

if __name__ == "__main__":
    base_folder = "/data/EAAT4_project/EAAT4/"
    data_loc = os.path.join(base_folder, "data/")
    meta_loc = os.path.join(base_folder, "data/metadata/epsc.csv")
    meta = pd.read_csv(meta_loc, sep="\t")
    meta = generate_filenames(meta, data_loc)
    files = meta["epsc"]
    #output_loc = input("enter output file folder name >>> ")
    output_loc = os.path.join(base_folder, "output", "epsc")
    print(os.getcwd())
